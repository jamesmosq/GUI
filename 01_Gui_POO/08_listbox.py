...
Métodos principales:
  listbox.insert(END, "texto")  → agregar elemento al final
  listbox.delete(índice)        → eliminar por posición
  listbox.get(índice)           → obtener texto de una posición
...
frame_entrada.pack()
entry = tk.Entry(frame_entrada, width=22, font=("Arial", 12), relief="solid", bd=1)
...
listbox = tk.Listbox(frame_lista, width=32, height=8,
                     font=("Arial", 11),
...
def agregar():
    ...
    messagebox.showinfo("Agregado", f"'{texto}' agregado.")
...
def ver_info():
    ...
    messagebox.showinfo("Info",
        f"Seleccionado: {listbox.get(sel[0])}\nTotal en lista: {listbox.size()}")
