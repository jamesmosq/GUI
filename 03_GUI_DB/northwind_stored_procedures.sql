-- POR AMOR A DIOS O A LOQUE UDS LE CREAN, LEAN TODO EL SCRIPT ANTES DE EJECUTARLO
-- ESTE SCRIPT CREA UNA BASE DE DATOS LLAMADA Northwindx (PORQUE YA EXISTE UNA LLAMADA Northwind)
-- MAS ABAJO ESTAN LOS STORED PROCEDURES QUE VAN A SERVIR PARA LAS OPERACIONES CRUD
CREATE DATABASE IF NOT EXISTS Northwindx;
USE Northwindx;
...
-- Seleccionar la base de datos Northwindx
-- ESTO LO VAS A EJECUTAR SOLO UNA VEZ, DESPUES DE TENER PREVIAMENTE LA BASE DE DATOS CREADA
-- PUEDEN EXISTIR VARIANTES DE ESTE SCRIPT DEPENDIENDO DE LA VERSION DE MYSQL O MARIADB
use Northwindx;

-- =====================================================
-- PROCEDIMIENTOS ALMACENADOS PARA NORTHWIND DATABASE
-- =====================================================
...
-- =====================================================
-- PROCEDIMIENTOS PARA PRODUCTOS (PRODUCTS)
-- =====================================================
...
BEGIN
    ...
    VALUES (p_ProductName, p_SupplierID, p_CategoryID, p_Unit, p_Price);

    COMMIT;

    SELECT LAST_INSERT_ID() AS ProductID, 'Producto insertado correctamente' AS Message;
END//
...
BEGIN
    ...
    -- Verificar si el producto está en uso en OrderDetails
    SELECT COUNT(*) INTO v_count FROM OrderDetails WHERE ProductID = p_ProductID;

    IF v_count > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se puede eliminar: el producto tiene órdenes asociadas';
    END IF;
...
-- =====================================================
-- PROCEDIMIENTOS PARA CLIENTES (CUSTOMERS)
-- =====================================================
...
BEGIN
    ...
    VALUES (p_CustomerName, p_ContactName, p_Address, p_City, p_PostalCode, p_Country);

    COMMIT;

    SELECT LAST_INSERT_ID() AS CustomerID, 'Cliente insertado correctamente' AS Message;
END//
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_count FROM Orders WHERE CustomerID = p_CustomerID;

    IF v_count > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se puede eliminar: el cliente tiene órdenes asociadas';
    END IF;
...
-- =====================================================
-- PROCEDIMIENTOS PARA EMPLEADOS (EMPLOYEES)
-- =====================================================
...
BEGIN
    ...
    SELECT LAST_INSERT_ID() AS EmployeeID, 'Empleado insertado correctamente' AS Message;
END//
...
BEGIN
    ...
    -- Verificar si el empleado tiene órdenes
    SELECT COUNT(*) INTO v_count FROM Orders WHERE EmployeeID = p_EmployeeID;

    IF v_count > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se puede eliminar: el empleado tiene órdenes asociadas';
    END IF;
...
-- OBTENER TODOS LOS PROVEEDORES (para combobox)
...
-- OBTENER TODAS LAS EMPRESAS DE ENVÍO (para combobox)
...
BEGIN
    SELECT p.ProductID, p.ProductName, s.SupplierName, c.CategoryName, p.Unit, p.Price
    FROM Products p
    LEFT JOIN Suppliers s ON p.SupplierID = s.SupplierID
    LEFT JOIN Categories c ON p.CategoryID = c.CategoryID
    WHERE p.ProductName LIKE CONCAT('%', p_SearchTerm, '%')
...
BEGIN
    SELECT CustomerID, CustomerName, ContactName, Address, City, PostalCode, Country
    FROM Customers
    WHERE CustomerName LIKE CONCAT('%', p_SearchTerm, '%')
       OR ContactName LIKE CONCAT('%', p_SearchTerm, '%')
    ORDER BY CustomerName;
END//
...
BEGIN
    SELECT EmployeeID, LastName, FirstName, BirthDate, Photo, Notes
    FROM Employees
    WHERE LastName LIKE CONCAT('%', p_SearchTerm, '%')
...
CALL sp_InsertProduct('Laptop Dell', 1, 1, 'Each', 899.99);
CALL sp_UpdateProduct(1, 'Laptop Dell XPS', 1, 1, 'Each', 999.99);
CALL sp_GetProduct(1);
CALL sp_GetAllProducts();
CALL sp_DeleteProduct(1);

-- EJEMPLOS DE CLIENTES:
CALL sp_InsertCustomer('Alfreds Futterkiste', 'Maria Anders', 'Obere Str. 57', 'Berlin', '12209', 'Germany');
CALL sp_UpdateCustomer(1, 'Alfreds Futterkiste Updated', 'Maria Anders', 'Obere Str. 57', 'Berlin', '12209', 'Germany');
CALL sp_GetCustomer(1);
CALL sp_GetAllCustomers();
CALL sp_DeleteCustomer(1);

-- EJEMPLOS DE EMPLEADOS:
CALL sp_InsertEmployee('Davolio', 'Nancy', '1968-12-08', 'nancy.jpg', 'Education includes a BA in psychology.');
CALL sp_UpdateEmployee(1, 'Davolio', 'Nancy', '1968-12-08', 'nancy_updated.jpg', 'Updated notes.');
CALL sp_GetEmployee(1);
CALL sp_GetAllEmployees();
CALL sp_DeleteEmployee(1);

-- EJEMPLOS DE BÚSQUEDA:
CALL sp_SearchProducts('Laptop');
CALL sp_SearchCustomers('Alfreds');
CALL sp_SearchEmployees('Nancy');
*/
